...
import forex.{BaseSpec, programs}
...
class RatesApiSpec extends BaseSpec {
    ...
    forAll(rateMap) { rates =>
      withApi(rates) { api =>
        rates.foreach { case (key, value) =>
          val request = GET(Uri.unsafeFromString(s"/rates?from=${key.from.entryName}&to=${key.to.entryName}")).unsafeRunSync()
          checkResponse(
        ...
        rates2.keys.filterNot(rates1.contains).foreach { key =>
          ...
          checkResponse(
            ...
            expectedFields = JsonObject(
              "from" -> Json.fromString(key.from.entryName),
              "to" -> Json.fromString(key.to.entryName),
              "message" -> Json.fromString(s"Rate is not available for currencies: ${key.from.entryName} to ${key.to.entryName}")
            ),
            responseF = api.routes(request).value
          )
        }
  ...
  private def dummyRateService(values: Map[Rate.Currencies, Rate]): RatesService[IO] =
    (currencies: Rate.Currencies) =>
      IO.pure(values.get(currencies).toRight(RateNotAvailable(currencies)))

  private def withApi(values: Map[Rate.Currencies, Rate])
                     (testCode: RatesApi[IO] => Any): Any =
